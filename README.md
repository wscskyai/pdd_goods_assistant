...
除了内置清理词之外，插件还支持你自己配置“自定义过滤词”。

适合这类场景：

- 某些店铺经常出现固定营销词
- 某些团队内部会在规格里加一些不想保留的标记
- 不想每次都去改 `src/content.js`

你可以在面板里打开 `自定义过滤词设置`，一行填写一个词。保存后会写入本地存储，后续处理时自动生效。

为了避免误删重要信息，插件在保存前会自动做以下校验：

- 少于 `2` 个字的词直接忽略
- 与必要关键字冲突的词会忽略
- 自动去重
- 保存后输入框会更新为“实际生效的过滤词列表”

这里的“必要关键字”主要包括：

- 颜色词
- 鞋子属性词，如 `网面 / 皮面 / 加绒 / 加棉 / 加厚`
- 尺码相关词，如 `码 / 内长 / XL / 均码`

这样可以最大程度避免因为误配置，导致真正有用的商品属性被删掉。
...
这个不是顶部参数输入框，而是面板中的一个附加设置入口，默认收起，放在“当前处理 X/Y页”右侧。
...
如果你有店铺自己的固定营销词、活动标签、内部标记词，这个功能会比频繁改代码更方便。
...
点击后会展开一个多行输入区，用来维护你自己的过滤词列表。保存后自动校验、自动去重，并写入本地。
